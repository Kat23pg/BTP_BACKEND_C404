...
CLASS zcl_insert_data_log IMPLEMENTATION.
  ...
  METHOD if_oo_adt_classrun~main.
    ...
    DELETE FROM ztb_travel2803_a.

    INSERT ztb_travel2803_a FROM (
    ...
    DELETE FROM ztbbooking2803_a.

    INSERT ztbbooking2803_a FROM (
      SELECT
        FROM /dmo/booking
        JOIN ztb_travel2803_a
          ON /dmo/booking~travel_id = ztb_travel2803_a~travel_id
        JOIN /dmo/travel
          ON /dmo/travel~travel_id = /dmo/booking~travel_id
        FIELDS
         " /dmo/booking~client            AS client,
          uuid( )                        AS booking_uuid,
          ztb_travel2803_a~travel_uuid     AS parent_uuid,
          ...
          ztb_travel2803_a~last_changed_at AS local_last_changed_at
    ).

    out->write( 'Adding Booking Supplements data' ).

    DELETE FROM ztbbksppl_2803_a.

    INSERT ztbbksppl_2803_a FROM (
      SELECT
        FROM /dmo/book_suppl AS supp
        JOIN ztb_travel2803_a AS trvl
          ON trvl~travel_id = supp~travel_id
        JOIN ztbbooking2803_a AS book
          ON book~parent_uuid = trvl~travel_uuid
